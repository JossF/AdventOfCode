...
test_state = pandas.read_csv(r"C:\Users\Joss\PycharmProjects\pythonProject\advent_of_code\2020\day_17_test.csv",
                             header=None)
run_state = pandas.read_csv(r"C:\Users\Joss\PycharmProjects\pythonProject\advent_of_code\2020\day_17.csv", header=None)
...
def neighbours(coords):
    unit_neighs = [(x1, y1, z1) for x1 in [-1, 0, 1] for y1 in [-1, 0, 1] for z1 in [-1, 0, 1]]
    return [t_add(unit_t, coords) for unit_t in unit_neighs if unit_t != (0, 0, 0)]
...
def load_t0(df, cache):
    ...
    for ind, row in df.iterrows():
        for x in range(len(row)):
            cache[0][(x, ind, 0)] = 1 if row.loc[x] == "#" else 0
...
def evolve_cache(cache):
    ...
    coords = [(xx, yy, zz) for xx in x_r for yy in y_r for zz in z_r]
    for xyz in coords:
        cube = cache[last_t].get(xyz, 0)
        neighs = [cache[last_t].get(n, 0) for n in neighbours(xyz)]
        cache[next_t][xyz] = determine_cube(cube, neighs)
...
def print_time_set(cached_coords):
    ...
    for z in range(min([t[2] for t in all_keys]), max([t[2] for t in all_keys]) + 1):
        ...
        print(pandas.DataFrame([[cached_coords[k] for k in all_keys if k[1:] == (y, z)] for y in yr]).replace(
            {0: ".", 1: "#"}))
        print("\n")
